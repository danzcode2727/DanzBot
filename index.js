...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ZidanGans = ["6289686610207@s.whatsapp.net"]
     ...
     const bayarLimit = (sender, amount) => {
        ...
        const limitAdd = (sender) => {
            ...
            if (args.match(/(https:\/\/chat.whatsapp.com)/gi)) {
                const check = await mek.inviteInfo(Zidan);
                if (!check) {
                    return
                } else {
		...
		case 'daftar':
					...
					if (isBanned) return reply(mess.only.benned)
					if (args.length < 1) return reply(`Parameter Salah\nCommand : ${prefix}daftar nama/umur\nContoh : ${prefix}daftar Zidan/17/Denpasar`)
					reg = `${body.slice(8)}`
					jeneng = reg.split("/")[0];
					umure = reg.split("/")[1];
					asal = reg.split("/")[2];
			...
			case 'menu':
                ...
                setTimeout( () => {
					costum(help(prefix, instagram, yt, name, pushname2, user, limitt), text, ZidanGans, replySet)
					}, 15000)
					setTimeout( () => {
            ...
            break  
                 ...
                 case 'kalkulator':
				   ...
				   if (isLimit(sender)) return reply(limits.limitend(pushname2))
				    ...
				    break 
				case 'owner':
                 client.sendMessage(from, {displayname: "DanzBot", vcard: vcard}, MessageType.contact, { quoted: mek})
...
// premium user
         case 'joox':
			if (isBanned) return reply(mess.only.benned)    
               ...
               if (anu.error) return reply(anu.error)
                 infomp3 = `「 *JOOX* 」\n\n*• Judul* : ${anu.result.judul}\n*• Album* : ${anu.result.album}\n*• Dipublikasi* : ${anu.result.dipublikasi}\n\n*TUNGGU SEBENTAR LAGI DIKIRIM MOHON JANGAN SPAM. SPAM? Ban*`
